  ...
  )
{
  ...
  for (int i = 1; i < MEDS_s; i++)
  {
    ...
    while (1 == 1) // redo generation for this index until success
    {
      ...
      time = -cpucycles();
      pmod_mat_mul_simd(G0prime, MEDS_k, MEDS_m * MEDS_n,
          Ti, MEDS_k, MEDS_k,
      ...
      int solve_res;
      time = -cpucycles();
      solve_res = solve(A, B_inv[i], G0prime, Amm);
      time += cpucycles();
      ...
      // printf("ENTERING PMOD MAT SYST CT\n");
      int sys_res;
      time = -cpucycles();
      sys_res = pmod_mat_syst_ct_simd(G[i], MEDS_k, MEDS_m*MEDS_n);
      time += cpucycles();
      sys_time += time;
  ...
  printf("(keygen) total time_mul_time:  %f   (%llu cycles)\n", total_mul_time / freq, total_mul_time);
  printf("(keygen) total solve_time:     %f   (%llu cycles)\n", solve_time / freq, solve_time);
  printf("(keygen) total pi_time:        %f   (%llu cycles)\n", pi_time / freq, pi_time);
  printf("(keygen) total sys_time:       %f   (%llu cycles)\n", sys_time / freq, sys_time);
  ...
  )
{
  ...
  for (int i = 0; i < MEDS_t; i++)
  {
    ...
    while (1 == 1)
    {
      ...
      pi_simd(G_tilde_ti, A_tilde[i], B_tilde[i], G_0);


      LOG_MAT_FMT(G_tilde_ti, MEDS_k, MEDS_m*MEDS_n, "G_tilde[%i]", i);

      if (pmod_mat_syst_ct_simd(G_tilde_ti, MEDS_k, MEDS_m*MEDS_n) == 0)
        break;
    }
    ...
    {
      {
        ...
        pmod_mat_mul_simd(mu, MEDS_m, MEDS_m, A_tilde[i], MEDS_m, MEDS_m, A_inv[h[i]], MEDS_m, MEDS_m);
...

      {
        ...
        pmod_mat_mul_simd(nu, MEDS_n, MEDS_n, B_inv[h[i]], MEDS_n, MEDS_n, B_tilde[i], MEDS_n, MEDS_n);
  ...
  )
{
  ...
  for (int i = 0; i < MEDS_t; i++)
  {
    if (h[i] > 0)
    {
      ...
      pi_simd(G_hat_i, mu, nu, G[h[i]]);


      LOG_MAT_FMT(G_hat_i, MEDS_k, MEDS_m*MEDS_n, "G_hat[%i]", i);

      pmod_mat_syst_ct_simd(G_hat_i, MEDS_k, MEDS_m*MEDS_n);
    ...
    else
    {
      while (1 == 1)
      {
        ...
        pi_simd(G_hat_i, A_tilde, B_tilde, G[0]);

        LOG_MAT_FMT(G_hat_i, MEDS_k, MEDS_m*MEDS_n, "G_hat[%i]", i);


        if (pmod_mat_syst_ct_simd(G_hat_i, MEDS_k, MEDS_m*MEDS_n) == 0)
        {
          LOG_MAT_FMT(G_hat_i, MEDS_k, MEDS_m*MEDS_n, "G_hat[%i]", i);
          break;
        }
  ...
  )
{
  ...
  for (int i = 1; i < MEDS_s; i++)
  {
    ...
    while (1 == 1) // redo generation for this index until success
    {
      ...
      int sys_res;
      time = -cpucycles();
      sys_res = pmod_mat_syst_ct(G[i], MEDS_k, MEDS_m*MEDS_n);
      time += cpucycles();
      sys_time += time;
